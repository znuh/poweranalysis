...
int hypo_templ_parse_line(char *line, hypo_template_t * t,
			  hypo_display_t * disp)
{
	...
	char *hypo_text = NULL;
	int res =
	    sscanf(line, "%s %d&%x %s %d&%x %d %x", sym_a, &ofs_a, &bits_a,
		   sym_b, &ofs_b, &bits_b, &disp_id, &color);
	...
	if (disp) {
		...
		if (hypo_text)
			sprintf(hypo_text, "HD(%s[%d]&%x, %s[%d]&%x)", sym_a,
				ofs_a, bits_a, sym_b, ofs_b, bits_b);
	}
